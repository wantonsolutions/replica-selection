...
public:
  /**
   * Create RpcServerHelper which will make life easier for people trying
   * to set up simulations with echos.
   *
   * \param port The port the server will wait on for incoming packets
   */
  RpcServerHelper (uint16_t port);
  ...
  /**
   * Create a RpcServerApplication on the specified Node.
   *
  ...
  /**
   * Create a RpcServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::RpcServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an RpcServer will be installed.
...
public:
  /**
   * Create RpcClientHelper which will make life easier for people trying
   ...
   */
  RpcClientHelper (Address ip, uint16_t port);
  /**
   * Create RpcClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
  ...
  /**
   * Given a pointer to a RpcClient application, set the data fill of the 
   ...
   *
   * \param app Smart pointer to the application (real type must be RpcClient).
   ...
   *
   * \param node The Ptr<Node> on which to create the RpcClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the RpcClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the 
   *          application created
   */
...
private:
  /**
   * Install an ns3::RpcClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an RpcClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
